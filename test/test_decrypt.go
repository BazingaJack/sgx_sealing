...
func test_decrypt(num int) error {
	// 开始计时
	startTime := time.Now()
...
func main() {
	...
	if err := test_decrypt(100); err != nil {
		ErrorLogger.Printf("Failed to decrypt: %v", err)
	} else {
		InfoLogger.Printf("Decrypt successfully!")
	}
